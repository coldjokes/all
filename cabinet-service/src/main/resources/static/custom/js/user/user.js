$(function(){
	...
	$(".source").append($.customOptions("source"));
//	$(".role").append($.customOptions("role"));
	
	//如果当前是管理员，需要把超管的角色过滤掉
	var roleHtml = "";
	const currentUserRole = SiteConfig.user.role;
	for(let x of Dict.getDict("role")){
		const code = x.code;
		if(code >= currentUserRole){
			roleHtml += `<option value=${x.code}>${x.text}</option>`;
		}
	}
      ...
      columns: [
    	    {
    		   ...
    		   formatter: function(value, row, index){
                ...
                align: "center",
                formatter: function(value, row, index){
                	return Dict.getText("source", value);
                }
            },{
                field: "status",
                title: "状态",
