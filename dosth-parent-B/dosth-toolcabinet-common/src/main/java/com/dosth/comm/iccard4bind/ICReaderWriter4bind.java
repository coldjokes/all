...
public class ICReaderWriter4bind implements IOReadWrite {
	
	private static final Logger logger = LoggerFactory.getLogger(ICReaderWriter4bind.class);
	...
	private ICCardSwipe4bind icUser;
	...
	private static ICReaderWriter4bind icObj = new ICReaderWriter4bind();

	private ICReaderWriter4bind() {
		sp = new ReadSerialPort(this);
	}

	public static ICReaderWriter4bind getICInstance(ICCardSwipe4bind icUser, String COM) {
		icObj.connCOM(COM);
		icObj.icUser = icUser;
		return icObj;
	}
...
//		byte[] data = {0x02,-0x80,0x00,0x42,0x13,0x00,0x05,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x67,-0x76,0x38,0x03};
	...
	public void processReadData(byte[] data) {
		...
		logger.info("ICReaderWriter4bind 接收到端口返回数据(长度为" + readStr.length() / 2 + " byte)：" + readStr);
	...
	private void parseICResponse(byte[] data) {
		...
		case READ: {
			if (isCurrCMDSucc) {
				// read done
				// Length: 4, Data: 02 80 00 11 Length: 19, Data: 00 B4 DD 1D 49 3D 08 04 00 62
				// 63 64 65 66 67 68 69 9D 03
		...
		public void run() {
			while (!stopFlag) {
				if (isOneHandShakeDone) {
					...
					prepareReadWrite();
					if (!"".equals(errMsg)) {
						icUser.onICSwipe("Read IC failed at " + errMsg);
					}
				}
			}
		}
