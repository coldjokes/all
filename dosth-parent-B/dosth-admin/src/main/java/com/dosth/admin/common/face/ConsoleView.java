package com.dosth.admin.common.face;
...
public class ConsoleView implements MatchingView {
	...
	public int showView(MatOfDMatch matches, MatOfKeyPoint srcKP, MatOfKeyPoint dstKP) {
		logger.info("%s Match Point(s)", matches.rows());
		...
		logger.info("Min Distance: %d", minDist);
		logger.info("Max Distance: %d", maxDist);
		...
		logger.info("%d GoodMatch Found", goodMatch.size());
